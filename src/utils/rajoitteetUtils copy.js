...
export function kayLapiKohdennukset(kohdennukset, lista = []) {
  return `<ul>${values(
    map(kohdennus => {
      return `<li>${kayLapiKohdennus(kohdennus, lista)}</li>`;
    }, kohdennukset)
  )}</ul>`;
}
...
export function getRajoiteListamuodossa(rajoiteId, rajoiteChangeObjects) {
  let rakenne = {};
  ...
  const listamuoto = join(" ", flatten(lapikaydytKohdennukset));

  console.info(lapikaydytKohdennukset);
