...
const suljeAlirajoitedialogi =
  () =>
  ({ getState, setState }) => {
    setState(
      assocPath(
        ["changeObjects", "unsaved", "alirajoitelomake"],
        [],
        getState()
      )
    );
...
const Store = createStore({
  ...
  actions: {
     ...
     */
    acceptRestriction:
      (sectionId, restrictionId, targetSectionId) =>
    ...
    addCriterion:
      (sectionId, kohdennusId, rajoiteId, kohdennusindeksipolku) =>
      ({ getState, setState }) => {
    ...
    createTextBoxChangeObject:
      (sectionId, koodiarvo, ankkuri, value = "") =>
      ({ getState, dispatch, setState }) => {
        if (sectionId) {
      ...
      ({ getState, setState }) => {
        ...
        const kohdennusChangeObjects = filter(
          cObj =>
            ...
            getAnchorPart(
              cObj.anchor,
              1 + 2 * length(kohdennusindeksipolku)
            ) === "kohdennukset",
          concat(
    ...
    setChanges:
      ...
      ({ getState, dispatch, setState }) => {
        ...
        const underRemovalFullPath = prepend(
          "underRemoval",
          anchorParts
        ).filter(Boolean);

        const savedByAnchor =
          path(savedFullPath, getState().changeObjects) || [];
    ...
    setRajoitelomakeChangeObjects:
      (rajoiteId, sectionId, targetSectionId) =>
      ({ getState, setState }) => {
