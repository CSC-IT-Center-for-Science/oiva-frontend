...
import Input from "../../../../components/00-atoms/Input";
import {
  useLocation,
  useRouteMatch,
  useHistory,
...
import Jarjestaja from "scenes/Koulutusmuodot/VapaaSivistystyo/components/Jarjestaja";
  ...
  return (
    <React.Fragment>
      <Helmet htmlAttributes={{ lang: intl.locale }}>
        <title>{intl.formatMessage(common.vstTitleName)} - Oiva</title>
      </Helmet>
      <BreadcrumbsItem to="/">
      ...
      <Router history={history}>
        <Switch>
          <Route
            path="/vapaa-sivistystyo/lupa/:lupaUuid"
            render={() => {
              return (
                <BaseData
                  keys={["lupaByUuid"]}
                  locale={intl.locale}
                  render={_props => <Jarjestaja {..._props} />}
                />
              );
            }}
          />
