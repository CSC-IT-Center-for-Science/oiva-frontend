...
const MuutospyyntoWizardTopThree = React.memo(() => {
    ...
    const [lastCheckedAsianumero, setLastCheckedAsianumero] = useState({asianumero: "", isDuplicate: false});

    const lomakedata = useMemo(
      () => ({formatMessage: intl.formatMessage, uuid, setLastCheckedAsianumero, lastCheckedAsianumero}),
      [intl, uuid, setLastCheckedAsianumero, lastCheckedAsianumero]
    );
